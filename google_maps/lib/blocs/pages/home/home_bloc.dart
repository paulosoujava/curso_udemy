...
class HomeBloc extends Bloc<HomeEvents, HomeState> {
  ...
  Polygon myTaps = Polygon(polygonId: PolygonId('my_taps_polygon'), fillColor: Colors.redAccent, strokeWidth: 3, strokeColor: Colors.white);
  ...
  _init() async {
    ...
    if (Platform.isAndroid) {
      ...
      _subscriptionGpsStatus = _locationPermissions.serviceStatus.listen((status) {
        add(
  ...
  Stream<HomeState> _mapOnMyLocationUpdate(OnMyLocationUpdate event) async* {
    ...
    yield this.state.copyWith(loading: false, myLocation: event.location, polylines: polylines, markers: markers);
  }
